...
from blockwart.domain.release_monitoring import (
    GITHUB_DOCUMENT_KEYS,
    MAX_GITHUB_OWNER_LENGTH,
    MAX_GITHUB_REPO_LENGTH,
    MAX_RELEASE_INTERVAL_SECONDS,
    MIN_RELEASE_INTERVAL_SECONDS,
    RELEASE_DOCUMENT_KEYS,
    RELEASE_PROVIDERS,
    normalize_service_release_monitoring,
    service_release_monitoring_violations,
    valid_github_owner,
    valid_github_repo,
)
...
def normalize_object_data(kind: str, data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if kind == "service":
        ...
        normalized = normalize_service_release_monitoring(normalized)
    return normalized
...
SERVICE_RELEASE_MONITORING_FIELDS = (
    _field(
        "release_monitoring",
        "object",
        allowed_keys=RELEASE_DOCUMENT_KEYS,
    ),
    _field("release_monitoring.enabled", "boolean"),
    _field(
        "release_monitoring.provider",
        "enum",
        enum_values=RELEASE_PROVIDERS,
        message="must be the supported public release provider",
    ),
    _field(
        "release_monitoring.interval_seconds",
        "integer",
        minimum=MIN_RELEASE_INTERVAL_SECONDS,
        maximum=MAX_RELEASE_INTERVAL_SECONDS,
        message=(
            f"must be an integer from {MIN_RELEASE_INTERVAL_SECONDS} "
            f"to {MAX_RELEASE_INTERVAL_SECONDS}"
        ),
    ),
    _field(
        "release_monitoring.github",
        "object",
        allowed_keys=GITHUB_DOCUMENT_KEYS,
    ),
    _field(
        "release_monitoring.github.owner",
        "string",
        strip_whitespace=True,
        min_length=1,
        max_length=MAX_GITHUB_OWNER_LENGTH,
    ),
    _field(
        "release_monitoring.github.repo",
        "string",
        strip_whitespace=True,
        min_length=1,
        max_length=MAX_GITHUB_REPO_LENGTH,
    ),
)
...
SERVICE_RELEASE_MONITORING_FORBIDDEN_FIELDS = (
    _field(
        "release_monitoring",
        "object",
        forbidden_message="is supported only for service objects",
...
def _validate_service_release_monitoring(data: Mapping[str, Any]) -> None:
    """Require one complete, canonical public GitHub identity when enabled."""

    violations = service_release_monitoring_violations(data)
    if not violations:
        return
    path, message = violations[0]
    raise ObjectSchemaError(
        ...
        message,
        violation=VIOLATION_REQUIRED_FIELD_MISSING,
        rule=public_rule_name(_validate_service_release_monitoring),
    )


def _reject_invalid_public_github_identity(data: Mapping[str, Any]) -> None:
    ...
    if isinstance(owner, str) and not valid_github_owner(owner.strip()):
        raise ObjectSchemaError(
            "data.release_monitoring.github.owner",
            "must be a canonical public GitHub owner name",
            violation=VIOLATION_VALUE_NOT_ALLOWED,
            rule=public_rule_name(_reject_invalid_public_github_identity),
        )
    if isinstance(repo, str) and not valid_github_repo(repo.strip()):
        raise ObjectSchemaError(
            "data.release_monitoring.github.repo",
            "must be a canonical public GitHub repository name",
            violation=VIOLATION_VALUE_NOT_ALLOWED,
            rule=public_rule_name(_reject_invalid_public_github_identity),
        )
...
SCHEMA_RULE_CONTRACTS: Mapping[str, str] = MappingProxyType(
    {
        ...
        "_validate_service_release_monitoring": (
            "data.release_monitoring is a closed service-only document. It "
            "requires an explicit enabled flag and, when enabled, one canonical "
            "public GitHub owner/repository identity. No host, URL, token, channel, "
            "or alternative installed-software source is expressible."
        ),
        "_reject_invalid_public_github_identity": (
            "The GitHub identity accepts only canonical bounded owner and repository "
            "segments; it cannot contain a host, scheme, port, path, or query."
        ),
...
SCHEMA_RULE_VIOLATIONS: Mapping[str, str] = MappingProxyType(
    {
        ...
        "_validate_service_release_monitoring": VIOLATION_REQUIRED_FIELD_MISSING,
        "_reject_invalid_public_github_identity": VIOLATION_VALUE_NOT_ALLOWED,
