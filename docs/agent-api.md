...
future objects; independent `project_creator = true` contributes no read
authority at all, because its only authority is creating root Projects on the
command surfaces. It can coexist with `catalog_viewer`;
