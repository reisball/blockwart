...
def test_database_cli_upgrades_then_checks_database(tmp_path: Path, capsys) -> None:
    ...
    assert "database_upgrade_ok revision=20260731_0012" in capsys.readouterr().out

    assert database_cli.main(["--database-url", database_url, "check"]) == 0
    assert "database_check_ok revision=20260731_0012" in capsys.readouterr().out

    assert database_cli.main(["--database-url", database_url, "integrity"]) == 0
    assert (
        "database_integrity_ok revision=20260731_0012 diagnostics=0"
